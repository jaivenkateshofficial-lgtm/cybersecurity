...
from networksecurity.utils.main_utils.main import load_yaml,write_yaml
...
class Datavalidation:
    ...
    def validate_data_drift(self,base_df:pd.DataFrame,current_df:pd.DataFrame):
        try:
            ...
            for columns in base_df.columns:
                no_data_drifted=True
                distance1=base_df[columns]
                distance2=current_df[columns]
                ks2=ks_2samp(distance1,distance2)
                distance_between_data_points=ks2.pvalue
                threshold=self.data_validation_config.distance_threshold
                if threshold>distance_between_data_points:
                    no_data_drifted=False
                    all_column_status=False
                report.update(
                    {
                    columns:{
                        'pvalue':distance_between_data_points,
                        'failure_status':not(no_data_drifted)
                    }
                    }
                )
    ...
    def Intilaize_data_validation(self):
        ...
        try:
            ...
            if status==False:
                ...
                if status==False:
                    ...
                    if status==False:
                        ...
                        os.makedirs(self.data_validation_config.data_validation_valid,exist_ok=True)
            ...
            train_df.to_csv(Invalid_train_file_path)
            test_df.to_csv(Invalid_test_file_path)
            data_validation_artifact=DataValidationArtifact(
                            validation_satus=status,
